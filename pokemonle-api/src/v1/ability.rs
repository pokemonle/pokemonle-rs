...
async fn get_pokemons_by_ability_id(
    State(state): State<AppState>,
    Path(ResourceId { id }): Path<ResourceId>,
    Query(PaginateQuery { page, per_page }): Query<PaginateQuery>,
    Query(Language { lang }): Query<Language>,
    ...
    Result::from(
        handler
            .get_pokemons_by_ability_id(id, page, per_page, lang)
            .await,
    )
}
...
pub fn routers() -> ApiRouter<AppState> {
    ...
    ApiRouter::new()
        .api_route(
            "/",
            get_with(list_abilities_with_pagination, |op| {
                response_with::<PaginatedResource<WithName<Model>>>(op).tag("ability")
            }),
        )
        .api_route(
            "/{id}",
            get_with(get_ability_by_id, |op| {
                response_with::<WithName<Model>>(op).tag("ability")
            }),
        )
        .api_route(
            "/{id}/pokemons",
